  ...
  return (
    ...
    >
      ...
      {palateOptions.map((region) => (
        <View key={region.key} className="mb-5">
          <Text className="mb-2 text-xs font-semibold uppercase tracking-wide text-gray-500">{region.label}</Text>
          <Pressable
          ...
          >
            <Text className={`text-sm font-medium ${selectedKey === region.key ? 'text-orange-600' : 'text-gray-800'}`}>
              All {region.label}
            </Text>
          </Pressable>
          <View className="flex-row flex-wrap gap-2">
            {region.children.map((c) => (
              ...
              >
                ...
                <Text className={`text-sm font-medium ${selectedKey === c.key ? 'text-orange-600' : 'text-gray-800'}`}>
                  {c.label}
                </Text>
              </Pressable>
            ))}
